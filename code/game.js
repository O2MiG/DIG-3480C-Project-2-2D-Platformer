...
// Power Ups
ZeroG.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
FlashG.prototype.act = function(step) {
  this.wobble += step * wobbleSpeed;
  var wobblePos = Math.sin(this.wobble) * wobbleDist;
  this.pos = this.basePos.plus(new Vector(0, wobblePos));
};
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null ||
      type == "lava" && this.status == "zeroGPU" ||
	  type == "lava" && this.status == "flashGPU" ||
      type == "spike" && this.status == null ||
	  type == "spike" && this.status == "zeroGPU" ||
	  type == "spike" && this.status == "flashGPU" ||
	  type == "missle" && this.status == null ||
	  type == "missle" && this.status == "zeroGPU" ||
	  type == "missle" && this.status == "flashGPU") {
	...
	coinsCollected -= coinsThisLevel;
	coinsThisLevel = 0;
	coinsDisplay = document.getElementById('coins').innerHTML='Coins collected: ' + coinsCollected;
